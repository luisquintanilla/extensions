...
public class DocumentExtractionClientExtensionsTests
{
    [Fact]
    public void GetService_InvalidArgs_Throws()
    {
        ...
        {
            _ = DocumentExtractionClientExtensions.GetService<object>(null!);
        });
    }
    ...
    {
        IDocumentExtractionClient? client = null;
        var content = new DataContent("data:application/pdf;base64,AQIDBA==");
        var ex1 = await Assert.ThrowsAsync<ArgumentNullException>(() => DocumentExtractionClientExtensions.ExtractAsync(client!, content));
        Assert.Equal("client", ex1.ParamName);

        using var testClient = new TestDocumentExtractionClient();
        DataContent? nullContent = null;
        var ex2 = await Assert.ThrowsAsync<ArgumentNullException>(() => DocumentExtractionClientExtensions.ExtractAsync(testClient, nullContent!));
    ...
    {
        // Arrange
        var expectedResponse = new DocumentExtractionResult([TestDocument.Page(1, "hello")]);
        string? observedMediaType = null;
        byte[]? observedBytes = null;

        using var client = new TestDocumentExtractionClient
        {
        ...
        // Act
        var result = await DocumentExtractionClientExtensions.ExtractAsync(client, new DataContent("data:application/pdf;base64,AQIDBA=="));
