...
public class DocumentExtractionPageResultExtensionsTests
{
    ...
    public void ToDocumentExtractionResult_AssemblesPagesAndUsage()
    {
        ...
        [
            new(new DocumentPage(2, [new DocumentBlock("page two")])) { PagesProcessed = 1, TotalPages = 2 },
            new(new DocumentPage(1, [new DocumentBlock("page one")], "# page one")) { Usage = new() { PagesProcessed = 2 } },
        ];
    ...
    public async Task ToDocumentExtractionResultAsync_AssemblesPagesAndUsageAsync()
    {
        ...
        Assert.Equal("page one\n\npage two", result.Text);
        Assert.Equal(2, result.Usage!.PagesProcessed);
        Assert.Null(result.Pages[0].Markdown);
        Assert.Equal("## page two", result.Pages[1].Markdown);
    }

    [Fact]
    public void ToDocumentExtractionResult_PreservesArrivalOrderForDuplicatePageNumbers()
    {
        DocumentPage[] pages = Enumerable.Range(0, 20)
            .Select(static _ => new DocumentPage(1, []))
            .ToArray();
        DocumentExtractionPageResult[] updates = pages
            .Select(static page => new DocumentExtractionPageResult(page))
            .ToArray();
