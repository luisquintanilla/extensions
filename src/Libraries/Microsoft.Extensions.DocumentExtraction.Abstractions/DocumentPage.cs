...
public class DocumentPage
{
    ...
    /// <param name="document">The semantic document fragment extracted from the page.</param>
    /// <param name="markdown">The exact provider-supplied Markdown for the page, when available.</param>
    /// <param name="evidence">Extraction evidence keyed by semantic node identifier.</param>
    [JsonConstructor]
    public DocumentPage(int pageNumber, Document document, string? markdown = null, IReadOnlyList<DocumentExtractionEvidence>? evidence = null)
    {
        if (pageNumber <= 0)
        {
        ...
        {
            if (item is null || !knownNodeIds.Contains(item.NodeId) || !evidenceNodeIds.Add(item.NodeId))
            {
                Throw.ArgumentException(nameof(evidence), "Evidence must reference each existing document node at most once.");
            }
        }
