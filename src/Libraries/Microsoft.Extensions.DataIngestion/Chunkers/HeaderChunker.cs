...
public sealed class HeaderChunker : IngestionChunker
{
    ...
    public HeaderChunker(IngestionChunkerOptions options) => _elementsChunker = new(options);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        foreach (DocumentNode element in document.Document.EnumerateContent())
        {
            ...
            {
                foreach (IngestionChunk chunk in SplitIntoChunks(document, headers, elements))
                {
                    yield return chunk;
                }

                int level = Math.Min(header.Level.GetValueOrDefault(1), MaxHeaderLevel);
    ...
    private IEnumerable<IngestionChunk> SplitIntoChunks(
        IngestionDocument document,
        ...
        List<DocumentNode> elements)
    {
        ...
        foreach (IngestionChunk chunk in _elementsChunker.Process(document, context, elements, contextNodes))
        {
            yield return chunk;
        }
