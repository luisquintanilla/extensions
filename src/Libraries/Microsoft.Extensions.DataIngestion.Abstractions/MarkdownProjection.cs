...
internal static class MarkdownProjection
{
    ...
    internal static string EscapeImageAlternativeText(string value)
        => value.Replace("\\", "\\\\")
            .Replace("]", "\\]");
        ...
        for (int row = 0; row < cellGrid.GetLength(0); row++)
        {
            ...
            for (int column = 0; column < cellGrid.GetLength(1); column++)
            {
                    ...
                    && cell is not null)
                {
                    ...
                    string tag = kind?.Contains("header", StringComparison.OrdinalIgnoreCase) is true
                        ? "th"
                        : "td";
                    builder.Append("    <");
                    builder.Append(tag);
                    ...
                    if (kind is not null && kind.Length > 0)
                    {
                        ...
                        builder.Append(EscapeHtml(kind));
                        builder.Append('"');
                    }
