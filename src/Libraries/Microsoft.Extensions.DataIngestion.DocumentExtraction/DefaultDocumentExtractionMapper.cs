...
internal static class DefaultDocumentExtractionMapper
{
        ...
        foreach (DocumentPage page in source.Pages)
        {
            if (page.PageNumber <= 0)
            {
                Throw.InvalidOperationException(
                    $"Document extraction page number '{page.PageNumber}' must be a positive one-based value.");
            }

            IngestionDocumentSection section = new() { PageNumber = page.PageNumber };
            foreach (DocumentElement sourceElement in page.Elements)
            {
                IngestionDocumentElement? mapped = MapElement(sourceElement, page.PageNumber);
                if (mapped is not null)
                {
                    section.Elements.Add(mapped);
                }
            }
    ...
    {
        IngestionDocumentElement? mapped = source switch
        {
...
#pragma warning restore CA1814
        ...
        foreach (DocumentTableCell sourceCell in source.Cells)
        {
            ...
            {
                IngestionDocumentElement? mapped = MapElement(sourceElement, pageNumber);
                if (mapped is not null)
                {
                    elements.Add(mapped);
                }
